...
Use this skill when the user wants Claude Code, Codex, or a similar coding agent to evaluate, scan, improve, or compare another skill, system prompt, agent file, repo policy file, or instruction bundle rather than directly solve the target task.
...
The thing being evaluated is the target instruction asset as used by the host agent platform, not the base model in isolation.
...
Standard Codex paths for quick analysis:
...
If none of the platform-standard paths exist, fall back to conservative repo discovery.
...
- a Codex user-skill directory such as `~/.codex/skills/`
...
2. Detect the likely platform context: `claude`, `codex`, or `generic`.
3. Start with platform-standard paths first.
4. Add medium-confidence candidates only when content signals justify them.
5. Normalize each discovery into `path`, `type`, `confidence`, `reason_signals`, `platform`, and `scope`.
6. Analyze the discovered targets and present a scoreboard.
...
- `Use skill-accuracy to quick-analyze this repo plus my global Codex skills`
...
Choose the most likely platform:

- `claude` when the repo primarily uses `CLAUDE.md`, `.claude/commands/`, or `.claude/agents/`
- `codex` when the repo or user path primarily uses `AGENTS.md`, `SKILL.md`, `skills/`, or `~/.codex/skills/`
- `generic` when neither platform is obvious
...
For Codex, group findings by scope:

- `scope=repo` for repo-local instruction assets
- `scope=user` for user-global Codex skills
...
When both repo-local and user-global Codex targets are present, keep them in separate groups in the output.
...
4. If the target is a repo, directory, or Codex skill root, discover and classify candidate instruction assets first.
...
- the user wants standard platform paths checked first
...
For Codex quick-analysis passes:

- group results into `Repo Targets` and `User Skill Targets` when both exist
- include `platform` and `scope` implicitly in the grouping or explicitly in the table
